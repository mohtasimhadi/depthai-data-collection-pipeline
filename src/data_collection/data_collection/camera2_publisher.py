...
class Camera2Publisher(Node):
    def __init__(self):
        super().__init__('camera2_publisher')
        self.publisher_rgb = self.create_publisher(Image, 'camera2/rgb', 10)
        self.publisher_left = self.create_publisher(Image, 'camera2/left', 10)
        self.publisher_right = self.create_publisher(Image, 'camera2/right', 10)
    ...
    def timer_callback(self):
        ...
        if self.qLeft.has():
            ...
            #self.get_logger().info("Publishing Left Mono frame")
        ...
        if self.qRight.has():
            ...
            #self.get_logger().info("Publishing Right Mono frame")
            image_message = self.bridge.cv2_to_imgmsg(frame, encoding="mono8")
            self.publisher_right.publish(image_message)

def main(args=None):
    rclpy.init(args=args)
    node = Camera2Publisher()
    rclpy.spin(node)
    rclpy.shutdown()
